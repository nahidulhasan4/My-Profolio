...
const Skill = ({ name, x, y }) => {
  return (
    <motion.div
      className="flex items-center justify-center rounded-full font-semibold bg-dark text-light py-3 px-6 shadaw-dark cursor-pointer absolute"
  ...
  return (
    <>
      ...
      <div className="w-full min-h-[60vh] sm:h-screen relative flex flex-wrap items-center justify-center rounded-[2rem] sm:rounded-full bg-circularLight p-6 sm:p-0 mt-8 sm:mt-0">
        <motion.div
          className="flex items-center justify-center rounded-full font-semibold bg-dark text-light p-6 sm:p-8 shadow-dark cursor-pointer mb-4 sm:mb-0"
          whileHover={{ scale: 1.05 }}
        >
          web
        </motion.div>
